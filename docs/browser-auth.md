...
Compatibility: configured auth is now enforced across JSON and HTML routes; missing credentials yield 401 (browser dashboard navigation redirects to login), insufficient roles yield 403, and inaccessible details yield 404. `/health` and `/login` are public. Existing token imports and CLI commands remain. No schema migration is needed because browser/parent/CSRF metadata uses the existing session table. Automatic unscoped session issuance and over-privileged role overrides are rejected.
