...
def provider_readiness(settings):
    requirements = {'hibp':('hibp_api_key',), 'dehashed':('dehashed_email','dehashed_api_key'), 'intelligencex':('intelligencex_api_key',)}
    binaries = {'whois':('whois_binary',), 'projectdiscovery':('subfinder_binary','httpx_binary')}
    rows = []
    for name in available_domain_provider_names():
        missing = ['ORGSCAN_'+key.upper() for key in requirements.get(name,()) if not getattr(settings,key)]
        missing += ['ORGSCAN_'+key.upper() for key in binaries.get(name,()) if not shutil.which(getattr(settings,key))]
        status = 'warning' if missing or name in {'all','all-enriched'} else 'ok'
        rows.append({'name':name,'status':status,'missing':missing,
                     'scope':'configuration only; no provider requests made'})
    return rows
