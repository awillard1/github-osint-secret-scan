...
def report_summary(storage, *, tenant_keys=None):
    ...
    for kind, model, label in (('organization', m.Organization, m.Organization.name),
        ...
        if endpoint_ids.get(kind):
            labels.update({(kind, str(identity)): name for identity, name in session.execute(
                select(model.id, label).where(conditions.get(model, True),
    ...
    return bounded_summary(dict(
        ...
        top_risky_findings=[safe_finding_fields(f, dict(id=f.id, title=f.title, severity=f.severity, confidence=f.confidence, risk_score=f.risk_score or 0, source_tool=f.source_tool, status=f.status)) for f in top],
